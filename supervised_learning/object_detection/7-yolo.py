...
class Yolo:
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        ...
        for i in range(len(boxes)):
            ...
            box_score = round(float(box_scores[i]), 2)
            cv2.rectangle(image, tuple(map(int, box[0:2])), tuple(map(int, box[2:4])), (255, 0, 0), 2)
            text = f"{box_class} {box_score}"
    ...
    def predict(self, folder_path):
        ...
        for image_file in files:
            ...
            image = cv2.imread(image_path)
            preprocessed_image = self.model.input.preprocess_input(image)
            preprocessed_image = np.expand_dims(preprocessed_image, axis=0)
            raw_predictions = self.model.predict(preprocessed_image)
            boxes, box_classes, box_scores = self.model.output.postprocess(raw_predictions, self.anchors, self.score_threshold, self.iou_threshold)
            self.show_boxes(image, boxes, box_classes, box_scores, image_file)
